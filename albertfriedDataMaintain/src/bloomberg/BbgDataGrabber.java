...
public class BbgDataGrabber {
	...
	public Map<String, ? extends TimeSeries> getData(TSType type,
			List<String> name, List<String> fields,
		...
		if (type == TSType.HisDiv) {
			...
			throw new Exception(
					"Type support now is historical dividend and historical security");
		}
	}
			...
			if (fields.numValues() > 0) {
				...
				for (int j = 0; j < fields.numValues(); ++j) {
					...
					String date = ParseDate.standardFromBbgDate(fieldData
							.getElementAsDate(BbgNames.DATE).toString());
					tmp.addNode(date);
					if (fieldData.hasElement(FieldValue.open))
						tmp.setOpen(date,
								fieldData.getElementAsFloat64(FieldValue.open));
					if (fieldData.hasElement(FieldValue.close))
						tmp.setClose(date,
								fieldData.getElementAsFloat64(FieldValue.close));
					if (fieldData.hasElement(FieldValue.high))
						tmp.setHigh(date,
								fieldData.getElementAsFloat64(FieldValue.high));
					if (fieldData.hasElement(FieldValue.low))
						tmp.setLow(date,
								fieldData.getElementAsFloat64(FieldValue.low));
					if (fieldData.hasElement(FieldValue.last))
						tmp.setLast(date,
								fieldData.getElementAsFloat64(FieldValue.last));
					if (fieldData.hasElement(FieldValue.volume))
						tmp.setVolume(date, fieldData
								.getElementAsFloat64(FieldValue.volume));
					if (fieldData.hasElement(FieldValue.pe))
						tmp.setPE(date,
								fieldData.getElementAsFloat64(FieldValue.pe));
					if (fieldData.hasElement(FieldValue.sharesOutstanding))
						tmp.setSharesOutstanding(
								date,
								fieldData
										.getElementAsInt64(FieldValue.sharesOutstanding));
					if (fieldData.hasElement(FieldValue.vwap))
						tmp.setVwap(date,
								fieldData.getElementAsFloat64(FieldValue.vwap));
					if (fieldData.hasElement(FieldValue.currentQuarterEEPS))
						tmp.setCurrentQuarterEEPS(
								date,
								fieldData
										.getElementAsFloat64(FieldValue.currentQuarterEEPS));
					if (fieldData.hasElement(FieldValue.callIV))
						tmp.setCallIV(date, fieldData
								.getElementAsFloat64(FieldValue.callIV));
					if (fieldData.hasElement(FieldValue.putIV))
						tmp.setPutIV(date,
								fieldData.getElementAsFloat64(FieldValue.putIV));
				}
			}
		}
		map.put(ticker, tmp);
	...
	public static void main(String[] args) throws Exception {
		...
		Map<String, HisDivTS> res2 = (Map<String, HisDivTS>) grabber.getData(
				TSType.HisDiv, name, Arrays.asList(FieldValue.divHis), null);
